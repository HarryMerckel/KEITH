...
class robot(object): # The robot class, featuring all the Pi-only based code
    ...
    def rightdetect(self):
        ...
        while self.encodersEnabled:
            ...
            if lastStatus == 0 and totalTime < 0.002:
                ...
                continue
        #print "Encoder 2: {0}".format(encoder2count)
        
        e1thread = Thread(target=leftdetect)
        e1thread.setDaemon(True)
        e1thread.start()
        
        e2thread = Thread(target=rightdetect)
    ...
    def startServo(self):
        ...
        self.launchServo = GPIO.PWM(servo, 200)
    ...
    def manualControl(self, arduino, mode=1):
        ...
        while True:
            ...
            try:
                ...
                switch = 500
            if mode == 1:
                self.controlServo(100 if switch > 1000 else 0)
            if mode == 2:
                GPIO.output(topLasers, 1 if switch > 1000 else 0)
            if mode == 3:
                GPIO.output(topLasers, 1 if switch > 1000 else 0)
            motor(left, right)
