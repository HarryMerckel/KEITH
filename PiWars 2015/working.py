...
class ArduinoReceiver(object):
    ...
    def __init__(self):
        ...
        self.correctionRatio        = 1.15
        self.upperDeadLimit         = 1450
        self.lowerDeadLimit         =  550
        self.middleDeadLower        =  950
        self.middleDeadUpper        = 1050
    ...
    def controllerRead(self):
        ...
        for i in range(3):
            ...
            try:
                controllerx = ((float(controller[3])-1000)*self.correctionRatio)+1000 # These scale up the range of the controller so the deadzones have more of an effect
                controllery = ((float(controller[1])-1000)*self.correctionRatio)+1000
            except:
                controllerx = 1000
                controllery = 1000
        ...
        motor1list = sorted(motor1list)
        motor2list = sorted(motor2list)
        #print(int(m1otorlist[1]), int(motor2list[1]), rawRadio)
        return int(motor1list[1]), int(motor2list[1]), controller # Taking the median value and returning it to the main loop
...
class robot(object): # The robot class, featuring all the Pi-only based code
    def leftdetect(self): # Wheel encoder switch detection - adds one each time it detects a change from white to black or vice versa.
        global encoder1count, encodersEnabled
        lastStatus = 0
        while True:
            ...
            status = True if totalTime > 0.0003 else False if totalTime < 0.0001 else None
            if lastStatus == 0 and status == False:
                encoder1count += 1
                lastStatus = 1
                print encoder1count
            if lastStatus == 1 and status == True:
                encoder1count += 1
                lastStatus = 0
            time.sleep(0.005)
    ...
    def rightdetect(self):
        global encoder2count, encodersEnabled
        lastStatus = 0
        print 'Rightdetect running'
        while True:
            #print 'Properly running'
            ...
            totalTime = time1-time0
            status = True if totalTime > 0.0003 else False if totalTime < 0.0001 else None
            print 'stop timing: {0}'.format(totalTime)
            if lastStatus == 0 and status == False:
                encoder2count += 1
                lastStatus = 1
            if lastStatus == 1 and status == True:
    ...
    def __init__(self):
        ...
        self.topLasers       = 23
        self.bottomLasers    = 24
        self.white           = 13
        self.green           = 18
        self.red             = 16

        GPIO.setup(self.topLasers,    GPIO.OUT)
        GPIO.setup(self.bottomLasers, GPIO.OUT)
        GPIO.setup(self.white,        GPIO.OUT)
        GPIO.setup(self.green,        GPIO.OUT)
        GPIO.setup(self.red,          GPIO.OUT)

        GPIO.output(self.topLasers, 0)
        GPIO.output(self.bottomLasers, 0)
        GPIO.output(self.white, 0)
        ...
        global encodersEnabled
        encodersEnabled = False
        """
        e1thread = Thread(target=self.leftdetect)
        e1thread.setDaemon(True)
        e1thread.start()
        
        e2thread = Thread(target=self.rightdetect)
        e2thread.setDaemon(True)
        e2thread.start()
        """
        """# MCP23017 I2C setup
    ...
    def startEncoders(self):
        global encoder1count, encoder2count, encodersEnabled
        encoder1count = 0
        encoder2count = 0
        ...
        if not encodersEnabled: # This is so multiple callbacks aren't added so this can be used to just reset the counts.
        ...
        while currentDistance < distance:
            ...
            if forwards:
                if difference == 0:
                    self.motor(10,10)
                if difference > 0:
                    self.motor(10-difference,10)
                if difference < 0:
                    self.motor(10,10-difference)
            else:
                if difference == 0:
                    self.motor(-100,-100)
                if difference > 0:
    ...
    def startServo(self):
        ...
        self.launchServo = GPIO.PWM(self.servoPin, 200)
    ...
    def distanceLoop(self, target1=5.5, target2=17, target3=30, target4=50): # The code for the Proximity Alert challenge.
        self.motor(0, 0)
        #GPIO.output(self.bottomLasers, 1)
        #GPIO.output(self.white, 1)
        time0 = time.time()
        while time.time() - time0 < 5.0:
            distance = self.ultrasonic(5)
            print distance
        GPIO.output(self.white, 1)
        distance = self.ultrasonic(5)
        print distance
        while distance >= target4: # Full speed ahead until less than target3 cm (20cm by default) from the wall.
            distance = self.ultrasonic(5)
            print distance
            #self.straightLine()
            self.motor(100,98)
    ...
    def manualControl(self, arduino, mode=1):
        ...
        while True:
            ...
            try:
                ...
                switch = 500
            if mode == 1:
                self.controlServo(30 if switch > 1000 else 0)
            if mode == 2:
                GPIO.output(self.topLasers, 1 if switch > 1000 else 0)
                if left < 0 or right < 0:
                    GPIO.output(self.red, 1)
                else: GPIO.output(self.red, 0)
            if mode == 3:
                GPIO.output(self.white, 1 if switch > 1000 else 0)
        ...
        while True:
            if True: #self.ultrasonic(5) > 10:
                ...
                if not l and not c and r:
                    self.motor(speed,-(speed*3)//4)
                    wasRight = True
                    continue
                if not l and not c and not r:
                    while not GPIO.input(self.lineMiddle) and (wasLeft or wasRight):
                        if wasLeft:
                            self.motor(-(3*speed)//4,speed)
                        elif wasRight:
                            self.motor(speed,-(3*speed)//4)
                    self.motor(speed,speed)
                    continue
                if l and c and r:
                    self.motor(0,0)
...
if __name__ == "__main__":
    try:
        ...
        #KEITH.manualControl(Receiver,hackMode)
        KEITH.distanceLoop()
        #KEITH.lineFollow()
    finally:
        print "Finishing"
