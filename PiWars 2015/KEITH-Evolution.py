...
class robot(object): # The robot class, featuring all the Pi-only based code
    def __init__(self):
        ...
        self.motor1forwards  = 19
        self.motor1backwards = 26
        self.motor2forwards  = 21
        self.motor2backwards = 20
        self.LED1            = 5
        self.buzzer          = 6
        self.trigger         = 14
        self.echo            = 4
        ...
        GPIO.setup(self.encoder1,GPIO.IN) # Wheel encoder inputs
    ...
    def startServo(self, name, pin, startPosition):
        GPIO.setup(servo, GPIO.OUT)

        name = GPIO.PWM(servo, 200)
        ...
        PWMposition = (startPosition//5)+10
        servoPWM.start(PWMposition)
    
    def controlServo(self, servo, position):
        # Set servo position, from 0 to 100
        PWMposition = (position/5)+10
        servo.ChangeDutyCycle(PWMposition)
